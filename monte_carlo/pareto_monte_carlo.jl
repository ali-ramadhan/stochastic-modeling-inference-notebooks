...
begin
	N = 10^4
	x̄ = [sample_pareto(n) for n in 1:N]
end

# ╔═╡ e786d3d1-b2d7-41d4-b602-9027b65cb1c9
md"""
The Monte Carlo method converges on the correct mean. $$\alpha^2 + \beta^2 = 1$$
"""

# ╔═╡ ef81dd91-aaf5-4f36-848d-7e1b4ab8aa77
begin
	x̄_mean = [mean(x̄[n]) for n in 1:N]
	...
	x̄_mean_rolling_avg = [mean(x̄_mean[1:n]) for n in 1:N]
	
	plot!(1:N, x̄_mean_rolling_avg,
		label="Rolling average",
		linewidth=2,
		color="red"
	)
	
	plot!([1, N], [pareto_mean(), pareto_mean()],
		label="True value",
		linestyle=:dash,
		linewidth=2,
		color="black",
...
begin
	...
	x̄_var_rolling_avg = [mean(x̄_var[1:n]) for n in 1:N-1]
	
	p2 = plot(2:N, x̄_var_rolling_avg,
		label="Rolling average",
		xlabel="Sample size n",
		linewidth=2,
		color="red",
	...
	plot(p1, p2, layout=grid(1, 2), legend=:outertop, fg_legend=:transparent)
end

# ╔═╡ 3ab2861e-7997-4e9f-bd06-cfaf01ea3080
begin
	x̄_skew = [skewness(x̄[n]) for n in 2:N]
	...
	x̄_skew_rolling_avg = [mean(x̄_skew[1:n]) for n in 1:N-1]
	
	plot!(2:N, x̄_skew_rolling_avg,
		label="Rolling average",
		linewidth=2,
		color="red"
	)
...
# ╔═╡ 577f8837-3c13-4d52-b044-79629ecef367
begin
	x̄_kurt = [kurtosis(x̄[n]) for n in 2:N]
	...
	x̄_kurt_rolling_avg = [mean(x̄_kurt[1:n]) for n in 1:N-1]
	
	plot!(2:N, x̄_kurt_rolling_avg,
		label="Rolling average",
		linewidth=2,
		color="red"
	)
